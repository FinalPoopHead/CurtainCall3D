...
namespace flt
{
	struct FLOATER_MATH_API Vector2f
	{
		...
		// x, y 우선순위로 비교.
		[[nodiscard]] std::partial_ordering operator<=>(const Vector2f& rhs) const noexcept
		{
		...
		constexpr Vector2f& operator*=(const float rhs) noexcept
		{
			x *= rhs;
			y *= rhs;
			return *this;
		...
		[[nodiscard]] constexpr Vector2f operator+(const Vector2f& rhs) const noexcept
		{
		...
		[[nodiscard]] constexpr Vector2f operator*(const float rhs) const noexcept
		{
			return Vector2f(*this) *= rhs;
		}
		[[nodiscard]] constexpr Vector2f operator/(const float rhs) const noexcept
		...
		[[nodiscard]] constexpr float NormPow() const noexcept
		{
			return x * x + y * y;
		}
		...
		[[nodiscard]] constexpr float Dot(const Vector2f& rhs) const noexcept
		{
